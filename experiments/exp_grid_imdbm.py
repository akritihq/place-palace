"""
Filtration grid on IMDB-M (Paper I, Proposition 4 validation + headline
accuracy).
...
Output:
    results/filtration_grid/imdbm_records.csv
    results/filtration_grid/imdbm_cache/seed{s}_fold{f}/{filt}__{tau}.npz

Run:
    python experiments/exp_grid_imdbm.py
    python experiments/exp_grid_imdbm.py --seeds 0 --tau-methods crossing
    python experiments/exp_grid_imdbm.py --force
"""
...
def run(seeds: list, tau_methods: list, n_scales_list: list,
    ...
    if filt_only:
        ...
        if not pool:
            raise ValueError(
                f'[IMDB-M] --filt-only {filt_only} matched 0 configs; '
                f'available: {sorted(c["name"] for c in _POOL)[:5]}...'
            )
        log(f'[IMDB-M] --filt-only restriction → {len(pool)}/{len(_POOL)} configs')
    log(f'\n[IMDB-M] filtration grid — {len(_ATOMIC_SINGLES)} atomic + '
    ...
    f_out.close()
    log(f'\n[IMDB-M] total wall-clock: {(time.time() - grand_t0)/3600:.2f} h')
    log(f'  → {output}')
...
def main():
    ...
    ap.add_argument('--output', type=str,
                    default=str(OUT_DIR / 'imdbm_records.csv'))
