"""
Filtration grid on NCI109 (Paper I, Proposition 4 validation + headline
accuracy).

Protocol: 10-fold stratified CV × 5 seeds × 64 filtration configs (shared
chemical pool — see pool_chemical.py) × {tau_method = crossing, proxy} ×
4 classifiers (Linear SVM, RBF SVM, WLK, Nearest Centroid).  Records
η̂ = Δ̂/√ℓ alongside every accuracy.

Extended persistence (H0+H1), following Wang (2019) chemical convention.

Output:
    results/filtration_grid/nci109_records.csv
    results/filtration_grid/nci109_cache/seed{s}_fold{f}/{filt}__{tau}.npz

Run:
    python experiments/exp_grid_nci109.py
    python experiments/exp_grid_nci109.py --seeds 0 --tau-methods crossing
    python experiments/exp_grid_nci109.py --force
"""
...
from experiments.pool_chemical import ATOMIC_SINGLES, ATOMIC_PAIRS, POOL


ROOT      = Path(__file__).resolve().parents[1]
OUT_DIR   = ROOT / 'results' / 'filtration_grid'
CACHE_DIR = OUT_DIR / 'nci109_cache'
...
# Filtration pool is shared across all chemical datasets (MUTAG, NCI1,
# NCI109, PTC, COX2, DHFR) — see experiments/pool_chemical.py for spec.
...
def run(seeds: list, tau_methods: list, n_scales_list: list,
    ...
    if filt_only:
        ...
        if not pool:
            raise ValueError(
                f'--filt-only {filt_only} matched 0 configs; '
                f'available names: {sorted(c["name"] for c in _POOL)[:5]}...'
            )
        log(f'[NCI109] --filt-only restriction → {len(pool)}/{len(_POOL)} configs')
    log(f'\n[NCI109] filtration grid — {len(pool)} configs')
    log(f'  seeds       : {seeds}')
    ...
    # ── diagram caches (extended persistence, H0+H1) ───────────────────
    log('\n── building / loading atomic diagram caches ──')
    ...
    f_out.close()
    log(f'\n[NCI109] total wall-clock: {(time.time() - grand_t0)/3600:.2f} h')
    log(f'  → {output}')
...
def main():
    ...
    ap.add_argument('--output', type=str,
                    default=str(OUT_DIR / 'nci109_records.csv'))
    ap.add_argument('--filt-only', nargs='+', default=None,
                    help='Restrict to listed filtration names (e.g. --filt-only hks_t10).')
    args = ap.parse_args()
